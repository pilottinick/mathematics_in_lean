...
theorem mul_left_cancel {a b c : G} (h : a * b = a * c) : b = c := by
  rw [← inv_mul_cancel_left a b, ← inv_mul_cancel_left a c, h]

-- exercises
theorem mul_one (a : G) : a * 1 = a := by
  have h : a⁻¹ * (a * (a⁻¹ * a)) = a⁻¹ * (1 * a) := by
    rw [← mul_assoc, one_mul, inv_mul_cancel, one_mul]
...
theorem mul_right_cancel {a b c : G} (h : a * b = c * b) : a = c := by
  rw [← mul_inv_cancel_right a b, ← mul_inv_cancel_right c b, h]
